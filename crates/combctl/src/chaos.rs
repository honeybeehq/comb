    ...
    for i in 0..iterations {
        ...
        let outcome = match op {
            0 | 1 => {
                ...
                store
                    .set_target(id, ref_name, payload_digest.clone(), fence)
                    .await
                    .map(|_| ())
            }
            2 => {
            ...
            _ => {
                ...
                if live_epoch > 0 {
                    let id = store.mint_operation();
                    match store
                        .set_target(id, ref_name, payload_digest.clone(), Some(live_epoch - 1))
                        .await
                    {
        ...
        let last_was_injected = match outcome {
            ...
            Err(e) => {
                if matches!(
                    e.downcast_ref::<comb_core::error::CoreError>(),
                    Some(comb_core::error::CoreError::BackendUnavailable(_))
                ) {
                    true
                } else {
