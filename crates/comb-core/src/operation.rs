...
impl StableKey {
    ...
    pub fn payload_hash(key: &DigestKey, payload: &[u8]) -> Digest {
        Self::payload_hash_batch(key, &[payload])
    }

    fn payload_hash_batch(key: &DigestKey, payloads: &[&[u8]]) -> Digest {
        domain_hash(key, "comb.log.stable-payload/v1", payloads)
    }
...
mod tests {
    ...
    fn stable_key_is_opaque_and_bounded() {
        ...
        assert!(StableKey::from_hex(&"aa".repeat(MAX_STABLE_KEY_BYTES + 1)).is_err());
    }
